...
void ThFunctionFineGrain(int partitionNumber,vector<int> vertexInPartition,int n);
...
*/
    ...
    vector<thread> v2;//n threads
    start_time = system_clock::now();
    for(int i=0;i<k;i++){
		v2.push_back(thread(ThFunctionFineGrain,i,std::ref(partitionVec[i]),n));
    }
    for(auto& t : v2) t.join();//iterate over the vector of threads auto for vector_iterator
...
void ThFunctionFineGrain(int partitionNumber,vector<int> vertexInPartition,int n){
	for(auto &currentVertex: vertexInPartition){
        if(ievertex[currentVertex]==0){//currentVertex is internal vertex
            ...
            //colour with minimum available currently
            vector<int> neighbour=getNeighbours(adj,n,currentVertex);
            vector<int> externalVertices;//To get list of boundary vertices.
            vector<int> currentlyLockedVertices;
            for(auto &neighbourVertex: neighbour){
                if(ievertex[neighbourVertex]==1){
                    externalVertices.push_back(neighbourVertex);
                }
            }
            externalVertices.push_back(currentVertex);
            std::sort(externalVertices.begin(), externalVertices.end());//Order the fineGrain locking of vertices
            
            /*Lock All the external vertices*/
            int countLocks=0;
            
            while (1){//GET ALL LOCKS FOR NEIGHBOURS and Vertex.IF THE VERTEX GETS ALL LOCKS THEN ONLY CONTINUE ELSE TRY  TO GET ALL LOCKS
				for(auto &Vertex: externalVertices){//Try to acquire lock for all vertices
                        if ( fineLock[Vertex].try_lock() ) {
                            countLocks++;
                            currentlyLockedVertices.push_back(Vertex);
                        }
                        else{
                            countLocks=0;
                            while(!currentlyLockedVertices.empty()){
                                fineLock[currentlyLockedVertices.back()].unlock();//Unlock the previously locked vertices
                                currentlyLockedVertices.pop_back();
                            }
                            break;
                        }
                    }
                if(countLocks==externalVertices.size()){
					break;
                }
            }
            ...
            //Unlock All the external vertices
            while(!currentlyLockedVertices.empty()){
                fineLock[currentlyLockedVertices.back()].unlock();//Unlock the previously locked vertices
                currentlyLockedVertices.pop_back();
            }
